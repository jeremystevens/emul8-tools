...
--Shows Opponent & Mac's Health and damage amounts.
--adelikat  

--*****************************************************************************
 -- fixed issues with the original script by Jeremy Stevens
  -- callNLua.Exceptions.LuaScriptException: [string "main"]:64: invalid arguments to method call
--*****************************************************************************
...
local OppKnockedDown = 0x0005 -- Opponent is on the canvas flag
local OppDown -- Stores contents of 0x0005
local OppDx = 130
local OppDy = 70
local OppWillGetUpWith = 0x039E -- Health that the opponent will get up with
...
local OHitValuex = 100
local OHitValuey = 100
...
--*****************************************************************************
    ...
    gui.text(0, 0, "Opponent: " .. EnemyHP)

    MacHP = mainmemory.read_u8(MHP)
    gui.text(0, 12, "Mac: " .. MacHP)
    ...
    if IsOppDown() then
        ...
        gui.text(0, 24, "Next health: " .. OppWillGet)
        gui.text(0, 36, "Will get up on: " .. OppGetUpCount)
    end
    ...
    if OppHitTimer > 0 then
        gui.text(0, 48, "Damage: " .. OppHitToDisplay)
    end
